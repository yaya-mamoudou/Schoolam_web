...
export default function UniversityCard ({uni}) {
  return (
    <div className="col-12 col-sm-6 col-md-4 col-lg-3">
          ...
          <Image objectFit="contain" className='align-self-center' width={50} height={50} src={uni.img}/>
          <h6 className='fw-bold text-center mt-2'>{uni.name}</h6>
          <div className='d-flex mt-2'>
            <i style={{width:20}} class="fas fa-hands text-secondary me-1"></i>
            <div>{uni.moto}</div>
          </div>
          <div className='d-flex mt-2'>
            <i style={{width:20}} class="fas fa-map-marker-alt text-secondary me-1"></i>
            <div>{uni.location}</div>
          </div>
          {uni.languages.map((lang,index)=> <button key={index} type='button' className={`btn btn-sm my-2 ${styles.language_btn}`}>{lang}</button>)}
