...
import { AppLayout, Button, Loader, MapUni, Title } from '../../../components/index';
  ...
  const ProgramTitle = ({ isOpen, title, price, fac }) => {
    return (
      <div
        ...
        style={{marginTop: isOpen && '1.2rem',transition:'all 1s'}}
      >
        <div className="d-flex align-items-center">
          ...
          <div className={`ms-0 ${styles.program_title}`}>
            ...
            >{`${title.toLocaleLowerCase()} (${fac?.toLocaleLowerCase()})`}</span>
          </div>
          <i
            className={`fas fa-caret-${
              isOpen ? 'down' : 'right'
            } d-block d-md-none ms-auto ms-md-0`}
          ></i>
        </div>
            ...
            <div className="border rounded p-3">
              ...
              {programs !== null ? (
                        ...
                        Object.keys(programs).map((data, index) => {
                          ...
                          return (
                            length > 0 && (
                              <Tab
                                key={index}
                                label={<TabComponent label={data} number={length} />}
                          ...
                          <TabPanel value={`${index + 1}`} key={index}>
                            {programs[data].map((program, index2) => (
                              <Collapsible
                                ...
                                trigger={
                                  <ProgramTitle
                                    fac={program.dep_id?.fact_id?.fact_name}
                                    price={program.fee}
                ...
                <div className="d-flex align-items-center justify-content-start mt-4 ">
                  {' '}
                  <Loader /> <span className="ms-3">Loading programs</span>
                </div>
              )}
            </div>
...
const TabComponent = ({ label, number }) => {
  return (
    <div className={`${styles.tab}`}>
      {label} <div className={`${styles.program_number}`}>{number}</div>
    </div>
  );
};
